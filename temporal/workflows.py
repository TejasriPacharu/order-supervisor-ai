...
class OrderSupervisorWorkflow:
    ...
    async def _main_loop(self, run_id, order_id, config):
        while self.status not in ("completed", "terminated"):
            has_events = await workflow.wait_condition(
                lambda: (
                    bool(self.pending_events)
                    or self.status in ("completed", "terminated")
                ),
            ...
            if self.status == "completed":
                ...
                await self._invoke_agent(
                    run_id, order_id, config, "terminal_event", events
                )
                break
            ...
            trigger = (
                "event"
                if has_events and self.pending_events
                else "scheduled_wakeup"
            )
            events = list(self.pending_events)
            self.pending_events.clear()
    ...
    async def add_instruction(self, instruction: str):
        ...
        self.pending_events.append(
            {"type": "instruction_added", "data": {"instruction": instruction}}
        )
